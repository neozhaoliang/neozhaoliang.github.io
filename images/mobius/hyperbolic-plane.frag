...
// Time constants
#define SPEED            (u_time * 2.5)
#define hue_speed        (u_time * 1.8)

bool iApply, iHyperbolic, parabolic, iElliptic, loxodromic;
Mobius iMobius;
...
vec3 hsv2rgb(vec3 hsv)
{
    ...
    return hsv.z * (0.63 * hsv.y * (cos(2.0 * PI *(hsv.x + p)) - 1.0) + 1.0);
}
...
void isometryParabolic(inout vec2 p) { p.x += u_time * polar_grid.x / 3.0; }
...
float sdScene1(vec3 p) { return sdPlane(p); }

// signed distance function for elliptic/hyperbolic case
float sdScene2(vec3 p) { return sdPlane(p); }
