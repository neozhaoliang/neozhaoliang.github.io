...
void main() {
    vec2 mousePos = (u_mouse.xy / u_resolution.xy) * 2.0 - 1.0;
    mousePos *= vec2(PI / 4.0, PI / 4.0).xy;
    vec2 screenPos = (gl_FragCoord.xy / u_resolution.xy) * 2.0 - 1.0;
    vec3 cameraPos = vec3(0.0, 0.0, -8.0);	
    vec3 cameraDir = vec3(0.0, 0.0, 1.0);
    vec3 planeU = vec3(1.0, 0.0, 0.0);
    vec3 planeV = vec3(0.0, u_resolution.y / u_resolution.x * 1.0, 0.0);
    ...
    vec3 color;
    if (material == 0.0) color = pow(texture2D(u_tex0, gl_FragCoord.xy/u_resolution.xy).rgb, vec3(2.2));
